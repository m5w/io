...
static constexpr char get_maximum_c(const char mask) {
  return ~static_cast<char>((static_cast<unsigned char>(~mask) + 1ull) >>
                            1ull);
}

template <std::size_t n> class Read {
public:
  static inline auto read(std::istream &is, std::uint64_t &x, const char c)
      -> decltype(is);
  static constexpr char mask = get_mask(n);
  static constexpr char maximum_c = get_maximum_c(Read<n>::mask);
...
public:
  static inline auto read(std::istream &is, std::uint64_t &x, const char c)
      -> decltype(is);
  static constexpr char maximum_c = get_maximum_c(get_mask(0ull));
};
...
public:
  static inline auto read(std::istream &is, std::uint64_t &x, const char c)
      -> decltype(is);
  static constexpr char mask = get_mask(1ull);
  static constexpr char maximum_c = get_maximum_c(Read<1ull>::mask);
};
...
public:
  static inline auto read(std::istream &is, std::uint64_t &x, const char c)
      -> decltype(is);
  static constexpr char maximum_c = get_maximum_c(get_mask(7ull));
};
...
static inline void
copy_least_significant_bytes(std::uint64_t &x, const char *s,
